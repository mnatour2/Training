...
const getActionCol = function (id) {
  return `<button class="btn btn-outline-warning btn-sm me-2 btn-edit" title="Edit" onclick="showEditModal(${id})">
...
function showEditModal(id) {
  ...
  document.getElementsByName("editUsername")[0].value = userToChange.username;
  document.getElementsByName("editEmail")[0].value = userToChange.email;
  document.getElementsByName("editMobileNumber")[0].value =
    userToChange.mobileNumber;
  ...
  clear([
    ...
    {
      input: "editMobileNumber",
      message: "mobileErrorMessage",
    },
  ]);

  confirmEditBtn.onclick = function () {
    validUsername = validateField(
      "editUsername",
      usernameRegex,
      "usernameErrorMessage"
    );
    validEmail = validateField("editEmail", email_regex, "emailErrorMessage");
    validMobile = validateField(
      "editMobileNumber",
      mobile_regex,
      "mobileErrorMessage"
    );
    ...
    if (validUsername && validEmail && validMobile) {
      ...
      userToChange.username =
        document.getElementsByName("editUsername")[0].value;
      userToChange.email = document.getElementsByName("editEmail")[0].value;
      userToChange.mobileNumber =
        document.getElementsByName("editMobileNumber")[0].value;

      document.querySelector(`tr[data-user-id="${id}"]`).innerHTML = `<td>${
        userToChange.id
      }</td><td>${userToChange.username}</td><td>${
        userToChange.email
      }</td><td>${userToChange.mobileNumber}</td><td>${getActionCol(
        userToChange.id
      )}</td>`;
      myEditModal.hide();
    }
